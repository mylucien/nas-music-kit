...
def load_settings() -> dict:
    defaults = {
        ...
        'alist_save_path': os.environ.get('ALIST_SAVE_PATH', '/music'),
    }
...
def alist_push(music_url: str, settings: dict) -> tuple:
    """向 Alist 提交离线下载任务，返回 (success, message)"""
    base_url  = settings.get('alist_url', '').rstrip('/')
    token     = settings.get('alist_token', '')
    save_path = settings.get('alist_save_path', '/music')
    if not base_url or not token:
        return False, 'Alist 未配置，请先在设置中填写地址和 Token'
    try:
        resp = requests.post(
            f"{base_url}/api/fs/add_offline_download",
            headers={'Authorization': token, 'Content-Type': 'application/json'},
            json={'path': save_path, 'urls': [music_url], 'tool': 'SimpleHttp', 'delete_policy': 'delete_on_upload_succeed'},
            timeout=15
        )
        result = resp.json()
        if result.get('code') == 200:
            return True, '已提交 Alist 离线下载'
        return False, result.get('message', f"Alist 错误 code={result.get('code')}")
...
@app.route('/api/lyric_get')
def lyric_get_endpoint():
    """返回歌词文本（供前端播放器实时显示，不写入磁盘）"""
    source   = request.args.get('source')
    track_id = request.args.get('id')
    is_vip   = request.args.get('vip') == '1'
    ...
    try:
        pkg = fetch_music_package(source, track_id, br='128', is_vip=is_vip)
        lyric = (pkg.get('lyric') or '') if pkg else ''
...
def _handle_download_core(source, track_id, name, artist, album, pic_id, br, download_lyric, is_vip):
    ...
    try:
        with requests.get(pkg['url'], headers=MEDIA_HEADERS, stream=True, timeout=60) as r:
            ...
            # ① 完整下载到内存，连接关闭前不写入 Rclone 目录
            buf = io.BytesIO()
            for chunk in r.iter_content(8192):
                buf.write(chunk)

        # ② with 块结束后连接已关闭，buf 是完整数据，一次性写入
...
def settings_endpoint():
    if request.method == 'GET':
        s = load_settings()
        token = s.get('alist_token', '')
        masked = (token[:8] + '***') if len(token) > 8 else ('***' if token else '')
        return jsonify({
            'alist_url':       s.get('alist_url', ''),
            'alist_token':     masked,
            'alist_save_path': s.get('alist_save_path', '/music'),
            'configured':      bool(s.get('alist_url') and s.get('alist_token')),
        })
    data = request.json or {}
    current = load_settings()
    new_token = data.get('alist_token', '')
    if '***' in new_token:
        new_token = current.get('alist_token', '')
    persist_settings({
        'alist_url':       data.get('alist_url', current.get('alist_url', '')).rstrip('/'),
        'alist_token':     new_token,
        'alist_save_path': data.get('alist_save_path', current.get('alist_save_path', '/music')),
    })
    return jsonify({'status': 'ok'})


@app.route('/api/alist/test', methods=['POST'])
def alist_test():
    data     = request.json or {}
    base_url = data.get('alist_url', '').rstrip('/')
    token    = data.get('alist_token', '')
    if '***' in token:
        token = load_settings().get('alist_token', '')
    if not base_url or not token:
        return jsonify({'ok': False, 'message': '请填写 Alist 地址和 Token'}), 400
    try:
        resp = requests.post(
            f"{base_url}/api/fs/list",
            headers={'Authorization': token, 'Content-Type': 'application/json'},
            json={'path': '/', 'refresh': False}, timeout=8
        )
        result = resp.json()
        if result.get('code') == 200:
            return jsonify({'ok': True, 'message': '连接成功 ✅'})
        return jsonify({'ok': False, 'message': result.get('message', '认证失败')}), 400
    except Exception as e:
        return jsonify({'ok': False, 'message': f'连接失败: {e}'}), 500


@app.route('/api/alist/push', methods=['POST'])
def alist_push_endpoint():
    """解析音乐 URL 后推送给 Alist 离线下载"""
    data     = request.json or {}
    source   = data.get('source')
    track_id = data.get('id')
    br       = data.get('br', '999')
    ...
    if not source or not track_id:
        return jsonify({'error': 'Missing required fields'}), 400
    pkg = fetch_music_package(source, track_id, br, is_vip)
    if not pkg or not pkg.get('url'):
        return jsonify({'error': '无法获取音乐链接'}), 404
    ok, msg = alist_push(pkg['url'], load_settings())
    if ok:
...
def alist_put(data, local_path_or_none, remote_filename: str, settings: dict) -> tuple:
    """
    通过 Alist PUT /api/fs/put 上传，返回 (success, message)
    data: 文件对象(流式) 或 bytes(歌词等小文件)
    local_path_or_none: 文件对象时传本地路径获取 size；bytes 传 None
    """
    base_url  = settings.get('alist_url', '').rstrip('/')
    token     = settings.get('alist_token', '')
    save_path = settings.get('alist_save_path', '/music').rstrip('/')
    if not base_url or not token:
        return False, 'Alist 未配置'
    remote_path = f"{save_path}/{remote_filename}"
    encoded     = urllib.parse.quote(remote_path, safe='/')
    content_length = str(os.path.getsize(local_path_or_none)) if local_path_or_none else str(len(data))
    try:
        resp = requests.put(
...
@app.route('/api/alist/upload', methods=['POST'])
def alist_upload_endpoint():
    """下载音频到内存 → 写ID3标签 → PUT上传到 Alist，歌词同步上传"""
    import tempfile
    data     = request.json or {}
    source   = data.get('source')
    track_id = data.get('id')
    ...
    if pkg.get('pic_url'):
        try:
            r = requests.get(pkg['pic_url'], headers=MEDIA_HEADERS, timeout=10)
            if r.status_code == 200:
                cover_bytes = r.content
        except: pass

    # 文件名清洗
    safe_n = re.sub(r'[\/:*?"<>|]', '', str(name))
    safe_a = re.sub(r'[\/:*?"<>|]', '', str(artist))
    fname  = f"{safe_a} - {safe_n}"

    # 全程使用临时文件，不经过 BytesIO，内存中不出现完整音频数据
    tmp_path = None
    ext = '.mp3'
    filename = f"{fname}{ext}"  # 先给默认值，后面会更新
    try:
        # 1. 流式下载直接写临时文件，不经过内存
        with requests.get(pkg['url'], headers=MEDIA_HEADERS, stream=True, timeout=60) as r:
            if r.status_code != 200:
                return jsonify({'error': f'CDN 响应错误: {r.status_code}'}), 502
            ctype = r.headers.get('content-type', '').lower()
            ext = mimetypes.guess_extension(ctype)
            if '.flac' in pkg['url'].lower() or 'audio/flac' in ctype: ext = '.flac'
            elif '.m4a' in pkg['url'].lower() or 'audio/mp4' in ctype: ext = '.m4a'
        ...
        # 2. 原地写标签，完成后释放封面
        write_tags(tmp_path, ext, name, artist, album, cover_bytes, pkg.get('lyric'))
        cover_bytes = None

        # 3. 用文件对象流式 PUT，requests 按块读取，内存中最多一个 chunk
        filename = f"{fname}{ext}"
        with open(tmp_path, 'rb') as f:
            ok, msg = alist_put(f, tmp_path, filename, settings)
        if not ok:
            return jsonify({'error': f'音频上传失败: {msg}'}), 500
    ...
    # PUT 歌词（纯文本，体积极小，bytes 直接传没问题）
    lrc_ok = False
    if pkg.get('lyric'):
        lrc_ok, _ = alist_put(pkg['lyric'].encode('utf-8'), None, f"{fname}.lrc", settings)

    return jsonify({
        'status':   'success',
        'filename': filename,
        'lrc':      'ok' if lrc_ok else 'skipped',
        'tags':     'ok',
    })
